...
const { createSprintReport } = require('../service/reportService');
...
exports.exportReport = (req, res) => {
  const sprint = sprintService.assertSprint(parseId(req.params.sprintId, 'sprintId'));
  const filename = `relatorio-sprint-${sprint.id}.pdf`;
  res.set({ 'Content-Type': 'application/pdf', 'Content-Disposition': `attachment; filename="${filename}"` });
  const report = createSprintReport(sprint);
  report.pipe(res);
  report.end();
};
...
exports.addAnnotation = (req, res) => res.status(201).json(sprintService.addAnnotation(parseId(req.params.sprintId, 'sprintId'), req.body));
exports.updateAnnotation = (req, res) => res.json(sprintService.updateAnnotation(parseId(req.params.sprintId, 'sprintId'), parseId(req.params.annotationId, 'annotationId'), req.body));
exports.removeAnnotation = (req, res) => {
  sprintService.removeAnnotation(parseId(req.params.sprintId, 'sprintId'), parseId(req.params.annotationId, 'annotationId'));
  res.status(204).send();
};
